...
export const SendActionWithResponse = <TResponse>(parameters: IActionParameters<TResponse>) => {
  ...
  const failureMessage = "Wystąpił nieoczekiwany błąd. Skonsultuj się z Administratorem";
...
export const SendActionWithResponseAndNotification = <TResponse>(parameters: IActionParametersWithNotification<TResponse>) => {
  const failureTitle = parameters.failureTitle ? parameters.failureTitle : "Nieoczekiwany błąd";
  const failureMessage = parameters.failureMessage ? parameters.failureMessage : "Wystąpił nieoczekiwany błąd. Skonsultuj się z Administratorem";
